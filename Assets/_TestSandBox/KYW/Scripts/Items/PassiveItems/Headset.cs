...
// 헤드셋 패시브 아이템
// 📍 위치: PassiveItems 폴더
// 🎯 목적: 사용 시 특별한 효과 부여 (예: 소리 감지, 특수 능력 등)
public class Headset : NetworkBehaviour, IItemInteraction
{
    [Header("Headset Settings")]
    [SerializeField] private string itemName = "Headset";
    ...
    public void OnUsePress(Vector2 mouseWorldPosition, Vector2 playerPosition)
    {
        ...
        {
            // 헤드셋 효과 적용 (인벤토리에 패시브 아이템 추가)
            playerInventory.AddPassiveItem(gameObject);
            Debug.Log($"[Headset] 플레이어 인벤토리에 헤드셋 효과 적용됨");
        }
        else
        {
            Debug.LogError("[Headset] PlayerInventory를 찾을 수 없습니다!");
        }
    ...
    public void OnPickedUp()
    {
        ...
        IsHeld = true;
        Debug.Log($"[Headset] {itemName} 픽업됨");
    }
    ...
    public void OnReleased()
    {
        ...
        IsHeld = false;
        Debug.Log($"[Headset] {itemName} 해제됨");
    }
    ...
    private void RemoveFromHand()
    {
        ...
        if (playerThrower != null)
        {
            ...
            Debug.Log($"[Headset] 기존 던지기 시스템으로 손에서 제거됨");
        }
        else
        {
            Debug.LogError("[Headset] PlayerObjectThrower를 찾을 수 없습니다!");
        }
        
        // 물리 활성화는 상관없음 (어차피 바로 Despawn)
        Runner.Despawn(Object);
        Debug.Log($"[Headset] {itemName} Despawn됨");
    }
} 